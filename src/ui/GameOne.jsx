...
console.log(levelId)
    return <g>
        ...
        <svg onPointerDown={()=>{
            ...
            switch (game.type) {
                case "game-fight":
                    setMaze(generateMaze(game.baseFight,game.levelFight * 2 + 1,"none"));
                    break;
                case "game-passing":
            ...
            useStore.getState().setGameId(selectLevel + "-" + levelId)
        ...
        <svg x={"50%"} transform={`translate(-${width > 1024?250:height > 660 && width < 500?130:170} 40)`}>
            <g transform={`scale(${height > 660 && width < 500?0.3:0.4})`}>
                ...
                <text  x={20} y={130} fill={"#A7EAF2"} fontSize={30}>СТАНЦИЯ: {game.baseFight} x {game.baseFight}</text>
        ...
        <svg x={"50%"} transform={`translate(${width > 1024?80:0} 40)`}>
            <g transform={`scale(${height > 660 && width < 500?0.3:0.4})`}>
                ...
                {quests.filter((el)=>el.sectorId === selectLevel).map((el,idx)=><g key={idx + "Прохождение-главы"}>
                    ...
                    <g>
                        {el.levels.filter((levF)=>levF.id === (selectLevel + "-" + levelId)).map((lev)=><g key={game.id + "Прохождение"}>
        ...
        <svg onPointerDown={()=>{
            useStore.getState().setGame({
                base:quests.filter((el)=>el.sectorId === selectLevel).find((el)=>el).levels.find((el)=>el.id === (selectLevel + "-" + levelId))?.gridSize,
                level:quests.filter((el)=>el.sectorId === selectLevel).find((el)=>el).levels.find((el)=>el.id === (selectLevel + "-" + levelId))?.maxTurns,
                baseFight:5,
                levelFight:3,
                type:"game-passing",
                page:"game_one",
                id:(selectLevel + "-" + levelId),
